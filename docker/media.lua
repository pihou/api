...
local bad_message = json.encode({
    ret_code = 9000,
    ret_msg = "something wrong happened in openresty2"
})
local bad_info = {
    ret_code = 9000,
    ret_msg = "something wrong happened in openresty1"
}
...
local function get_meta_data()
    ...
    while true do
        ...
        if typ == "header" then
            if string.lower(res[1]) == "content-disposition" then
            ...
            elseif string.lower(res[1]) == "content-type" then
